...
export default class VecteurNormalADeuxAutresVecteurs extends Exercice {
  ...
  nouvelleVersion () {
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      texte = 'L\'espace est muni d\'un repère orthonormé $(O ; \\vec{\\imath}, \\vec{\\jmath}, \\vec{k})$.<br> Déterminer les réels $a$ et $b$, tels que '
      if (k === 1) { texte += `$\\overrightarrow{n}\\begin{pmatrix}${n1}\\\\a\\\\b\\end{pmatrix}$` }
      if (k === 2) { texte += `$\\overrightarrow{n}\\begin{pmatrix}a\\\\${n2}\\\\b\\end{pmatrix}$` }
      if (k === 3) { texte += `$\\overrightarrow{n}\\begin{pmatrix}a\\\\b\\\\${n3}\\end{pmatrix}$` }
      ...
      if (this.interactif) {
        texte += '<br><br> $\\overrightarrow{n}$ ' + remplisLesBlancs(this, i, '\\begin{pmatrix}%{champ1}\\\\%{champ2}\\\\%{champ3}\\end{pmatrix}.')
      } else {
        texte += '.'
      }
      handleAnswers(this, i, { champ1: { value: n1 }, champ2: { value: n2 }, champ3: { value: n3 } })

      texteCorr = 'On sait que '
      if (k === 1) { texteCorr += `$\\overrightarrow{n}\\begin{pmatrix}${n1}\\\\a\\\\b\\end{pmatrix}$` }
      if (k === 2) { texteCorr += `$\\overrightarrow{n}\\begin{pmatrix}a\\\\${n2}\\\\b\\end{pmatrix}$` }
      if (k === 3) { texteCorr += `$\\overrightarrow{n}\\begin{pmatrix}a\\\\b\\\\${n3}\\end{pmatrix}$` }
      texteCorr += 'est un vecteur normal au plan engendré par $\\vec{u}$ et $\\vec{v}$. <br>'
      texteCorr += '$\\vec n$ est donc orthogonal à tout vecteur de ce plan, ce qui implique que $\\vec n$ est orthogonal à $\\vec u$ et à $\\vec v$.<br>'
      texteCorr += 'Ainsi, on a necessairement : $\\vec{n} \\cdot \\vec{u} = 0 \\quad (L_1)\\quad\\quad\\text{et} \\quad \\vec{n} \\cdot \\vec{v} = 0\\quad(L_2)$.'
      texteCorr += '<br>Traduisons ces conditions dans un système de 2 équations à 2 inconnues, que l\'on résout ici par combinaisons linéaires (d\'autres méthodes sont possibles) :<br>'
      if (k === 1) {
