...
export default class nomExercice extends Exercice {
  ...
  nouvelleVersion () {
    ...
    if (k === 4) { kuplet = 'quadruplets' }
    if (k === 5) { kuplet = 'quintuplets' }
    if (k >= 6) { kuplet = `${k}-uplets` }
    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      switch (listeTypeQuestions[i]) {
        case 'type1':
          ...
          if (n === 3) { texteCorr += '$E=\\left\\{1;2;3\\right\\}$.<br>' }
          if (n === 4) { texteCorr += '$E=\\left\\{1;2;3;4\\right\\}$.<br>' }
          if (n > 4) { texteCorr += `$E=\\left\\{1;2;\\ldots;${n}\\right\\}$.<br> ` }
          texteCorr += 'Le nombre de $k$-uplet d\'un ensemble fini $E$ est $\\mathrm{Card}(E)^k$.<br>'
        ...
        case 'type3':
          ...
          texteCorr += 'On sait que le nombre de combinaisons, de k éléments parmi n, vaut : $\\dbinom{n}{k} =\\dfrac{n~!}{k~!(n-k)~!}$.<br>'
          ...
          texteCorr += `Il y a donc $${miseEnEvidence(combinaison)}$ tirages possibles.<br>`
          reponse = `${combinaison}`
          break
        case 'type4':
          ...
          texteCorr += `On peut donc créer $${miseEnEvidence(2 ** n)}$ sous-parties de $E$.<br>`
          reponse = `${2 ** n}`
          break
      }

      handleAnswers(this, i, { reponse: { value: reponse}})
      texte += ajouteChampTexteMathLive(this, i, KeyboardType.clavierDeBase, { texteAvant: 'Ma réponse : ' })
