...
export const titre = 'QCM : Calculer une probabilité avec la loi binomiale'
...
export default class Binomiale extends ExerciceQcm {
  versionOriginale: () => void = () => {
    ...
    texte += 'On estime que les lancers sont effectués dans des conditions identiques et qu\'ils sont indépendants les uns des autres.<br>'
    texte += `Déterminer la probabilité qu'il place exactement $${texNombre(k)}$ fléchettes au centre de la cible.`
    this.enonce = texte

    let correction = 'On note $X$ la variable aléatoire qui compte le nombre de fléchettes qui atteignent la cible.<br>'
    ...
    correction += 'La probabilité qu\'il place exactement $k$ fléchettes dans la cible est donnée par la relation :<br>'
    correction += '$P(X=k)=\\displaystyle\\binom{n}{k} \\times p^k \\times (1-p)^{n-k}$<br>'
