...
import { rotation, symetrieAxiale, translation } from '../../../lib/2d/transformations'
...
export default class MetropoleJ25EX3Q2 extends ExerciceQcmA {
  private appliquerLesValeurs (transfo:number): void {
    ...
    const cocotte = polygone([point1, point2, point3, point4, point5, point7, point8, point9, point10])
    cocotte.epaisseur = 2
    ...
    const fig1 = texteParPosition('Figure 1', 2, 5.5, 0, 'black', 0.8, 'milieu', false)
    const fig2 = texteParPosition('Figure 2', 10, 1.5, 0, 'black', 0.8, 'milieu', false)
    const grid = grille(0, -1, 12, 11, 'black', 0.5, 1)
    const objets = [grid, fig1, fig2, cocotte, cocotte2]
    const figure = mathalea2d(Object.assign({ style: 'display: inline' }, fixeBordures(objets)), objets)
    const transfos = [
      'Une symétrie axiale',
      'Une symétrie centrale',
      'Une rotation',
      'Une translation']
    this.reponses = [transfos[transfo], transfos[(transfo + 1) % 4], transfos[(transfo + 2) % 4], transfos[(transfo + 3) % 4]
    ]
    this.enonce = 'Quelle transformation permet de passer de la figure 1 à la figure 2 ?' + figure
