...
import { arete3d, CodageAngleDroit3D, point3d, polygone3d, pyramide3d } from '../../../modules/3d'
...
export default class MetropoleJ25EX3Q6 extends ExerciceQcmA {
  private appliquerLesValeurs (a:number, b:number, c:number): void {
    ...
    const l = placeLatexSurSegment(`${texNombre(b, 1)}\\text{ cm}`, point2.c2d, point1.c2d, { letterSize: 'scriptsize' })
    const p = placeLatexSurSegment(`${texNombre(a, 1)}\\text{ cm}`, point3.c2d, point2.c2d, { letterSize: 'scriptsize' })
    const h = latex2d(`${texNombre(c, 1)}\\text{ cm}`, 4.7, 3, { letterSize: 'scriptsize' })

    const objets = [pyr.c2d, hauteur.c2d, diag1.c2d, diag2.c2d, ang, l, p, h]
    const figure = mathalea2d(Object.assign({ style: 'display: inline-bloc' }, fixeBordures(objets)), objets)
    ...
    this.enonce = 'Quelle est lle volume de cette pyramide à base rectangulaire ?' + figure
