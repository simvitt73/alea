    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      switch (type[i]) {
            ...
            case 2:
              {
                ...
                seb = SchemaEnBoite.additionPartiesTout(somme, 2, [nb1, undefined])
              }
              break
            default:
              texte = `${lea} a ${somme} bonbons. Après en avoir donné à ${eric}, il lui en reste ${nb1}. Combien de bonbons a-t-elle donnés ?<br>`
              seb = SchemaEnBoite.additionPartiesTout(somme, 2, [undefined, nb1])
        ...
        case 5: { // Division dont on cherche le reste
          ...
          break
        case 6: { // Un exemple avec barres séparées et flèche
...
Combien ${prenoms[0]} a-t-elle dépensé de plus que sa sœur ?<br>`
          ...
          break
        case 7: { // additif avec une barre et une accolade
          ...
          seb = new SchemaEnBoite({
            ...
            lignes: [{
              ...
              barres: range(nb1 - 1).map(i => {
                const barre = Object.assign({ length: 2, color: 'magenta', options: { color: 'white' } }, {
                  type: i === 0 ? 'flèche' as 'flèche' : 'boite' as 'boite',
                  content: i === 0 ? '? cm' : '',
                })
                return barre
              }
          ...
          seb = new SchemaEnBoite({
            lignes: [{
              ...
              barres: range(nb2 - 1).map(i => {
                const barre = Object.assign({ length: 3, type: 'boite' as 'boite', color: 'magenta', content: '', options: { style: `min-height: ${(2.5 * 1).toFixed(1)}em;` } }, {})
                return barre
              })
            }],
            bottomBraces: [{
