  ...
  nouvelleVersion (numeroExercice: number) {
    ...
    for (let i = 0, a, b, k, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) { // On limite le nombre d'essais pour chercher des valeurs nouvelles
      ...
      if (this.sup2) {
        ...
        const facteur2 = facteurs[rang2] === '\\ldots\\ldots\\ldots' ? facteurs[rang2] : ecritureParentheseSiNegatif(Number(facteurs[rang2]))
        texte = '$ ' + facteurs[rang1] + ' \\times ' + facteur2 + ' = ' + (a * b) + ' $'
        texteCorr = '$ ' + facteurs[rang1 + 2] + ' \\times ' + ecritureParentheseSiNegatif(Number(facteurs[rang2 + 2])) + ' = ' + (a * b) + ' $'
      } else {
