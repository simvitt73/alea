...
export default class SubstituerDansUneExpressionLitterale extends Exercice {
  ...
  nouvelleVersion () {
    console.log('simplify.rules', simplify.rules)
    this.listeQuestions = []
    ...
    for (let i = 0, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      texte = `Pour $x=${x}$,${sp(1)} calculer : <br>`// mettre $~$ revient à faire créer par Katex une armada de span dans le html alors qu'un &nbsp suffit (c'est ce que va faire sp(1))
      texte += `$${toTex(expressionX)}$` // '$' + `${toTex(expressionX)}` + '$' ici la concaténation est superflue si tu utilises un littéral, tu peux tout mettre dedans
