...
export default class PoucentageE extends ExerciceSimple {
  ...
  nouvelleVersion () {
    ...
    switch (choice(['a', 'b', 'c', 'd', 'e'])) { //
      case 'a':
       ...
       Il baisse de $${b}\\,\\%$. `
        if (!this.versionQcm) { this.question += '  Quel est son nouveau prix ? ' } else { this.question += 'Son nouveau prix est : ' }
        this.distracteurs = [`$${texNombre(a + (b * a) / 100, 2)} $ €`,
  ...
  `)
    ...
    Puisque $20\\,\\%$  est deux fois plus grand que $10\\,\\%$ ,  $20\\,\\%$  de $${a}$ est égal à $2\\times ${a / 10}=${2 * a / 10}$.<br>
        ...
        this.reponse = this.versionQcm ? `$${texNombre(a - (b * a) / 100)}$ €` : a - (b * a) / 100
...
`)
        ...
        this.reponse = this.versionQcm ? `$${texNombre(a - (b * a) / 100, 2)}$ €` : a - (b * a) / 100
