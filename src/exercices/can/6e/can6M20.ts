...
export const titre = 'Connaître les relations entre unités consécutives de longueurs'
...
export default class ConversionUnitesMetriquesConsecutives extends ExerciceSimple {
  ...
  nouvelleVersion () {
    ...
    this.reponse = choixPlusGdVerPlusPetit ? 10 : new FractionEtendue(1, 10).texFraction
    this.correction = `$1$ ${unite1} $= ${miseEnEvidence(this.reponse)}$ ${unite2}`
    if (!choixPlusGdVerPlusPetit) this.correction += `$= ${miseEnEvidence(texNombre(0.1))}$ ${unite2}`
