...
export default function DoubleOuTripleDecimal () {
  ...
  this.nouvelleVersion = function () {
    ...
    if (choice([true, false])) {
      ...
      this.correction = `Le triple de $${texNombre(c, 1)}$ est $3 \\times ${texNombre(c, 1)}=${miseEnEvidence(texNombre(this.reponse, 1))}$.<br>`
  ...
  On en fait la somme : $${3 * e}+${texNombre(d.mul(3), 1)}$, ce qui donne le résultat $${texNombre(this.reponse, 1)}$.
      ...
      this.correction = `Le double de $${texNombre(c, 1)}$ est $2 \\times ${texNombre(c, 1)}=${miseEnEvidence(texNombre(this.reponse, 1))}$.<br>`
      this.correction += texteEnCouleur(`
      <br> Mentalement : <br>
  On décompose $${texNombre(c, 1)}$ en $${e}+${texNombre(d, 1)}$. <br>
