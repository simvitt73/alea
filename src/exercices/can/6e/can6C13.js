...
export default function FSomme2Decimaux () {
  ...
  this.nouvelleVersion = function () {
    ...
    if (choice([true, false])) {
      ...
      this.correction = `$${texNombre(n2, 1)}+${texNombre(e - a, 0)}=${miseEnEvidence(texNombre(this.reponse, 1))}$<br>`
      this.correction += texteEnCouleur(`
      <br> Mentalement : <br>
      On fait la somme des parties entières des deux nombres : $${a}+${e - a}=${e}$, puis on ajoute les dixièmes. On obtient :<br>
      $${e}+${texNombre(n1, 1)}=${texNombre(this.reponse, 1)}$`, bleuMathalea)
    } else {
      a = randint(1, 9)
      b = randint(3, 5)
      c = randint(1, 9)
      ...
      this.correction = `$${texNombre(n1.plus(a), 1)}+${texNombre(n2.plus(c), 1)}=${miseEnEvidence(texNombre(this.reponse, 1))}$<br>`
      this.correction += texteEnCouleur(`
        <br> Mentalement : <br>
