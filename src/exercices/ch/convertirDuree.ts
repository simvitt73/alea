...
export default class ConvertirDuree extends Exercice {
  ...
  nouvelleVersion (): void {
    ...
    this.consigne = ' Arrondir les résultats avec à 2 chiffres après la virgule.'
    ...
    for (let i = 0, cpt = 0, champIndex = 0; i < this.nbQuestions && cpt < 50; i++) {
      ...
      if (this.sup2) {
        ...
        totalSecondes = randint(60, 3 * 3600) // Au moins 1 minute
      }
      ...
      if (uniteDepart === 'hhmmss') {
        ...
        if (Math.round(valeurBrute * 100) !== valeurBrute * 100) {
          estApproximation = true
          // Arrondir à 2 décimales pour la cohérence affichage/correction
          valeurDepart = Math.round(valeurBrute * 100) / 100
        } else {
          valeurDepart = valeurBrute
        }
      }
      ...
      if (uniteArrivee === 'hhmmss') {
        ...
        if (estApproximation && uniteDepart !== 'hhmmss') {
      ...
      if (this.correctionDetaillee) {
        texteCorr = this.genererCorrectionDetaillee(typeDeQuestion, valeurDepart, reponse, uniteDepart, uniteArrivee, estApproximation) + '<br><br>'
      } 
      texteCorr += this.genererCorrectionSimple(valeurDepart, reponse, uniteDepart, uniteArrivee, estApproximation)
  ...
  private genererCorrectionDetaillee(typeDeQuestion: number, valeurDepart: number | string, reponse: number | string, uniteDepart: string, uniteArrivee: string, estApproximation: boolean = false): string {
    let texteCorr = ''
    ...
    switch (typeDeQuestion) {
      case 1: // hhmmss → hdec
        ...
        if (m1 > 0) {
          ...
          texteCorr += ` minutes : $${m1}\\text{\\,min} = \\dfrac{${m1}}{60}\\text{\\,h}$;`
        }
      ...
      case 2: // hhmmss → mindec
      {
        ...
        if (estApproximation) {
          texteCorr += ` \\approx ${miseEnEvidence(texNombre(totalMinutes, 2))}\\text{\\,min}$`
        } else {
          texteCorr += ` = ${miseEnEvidence(texNombre(totalMinutes, 2))}\\text{\\,min}$`
        }}
        break
  ...
  private genererCorrectionSimple(valeurDepart: number | string, reponse: number | string, uniteDepart: string, uniteArrivee: string, estApproximation: boolean = false): string {
    let texteCorr = '$'
    ...
    if (typeof reponse === 'string') {
      ...
      const reponseAvecUnites = reponse.replace(/(\d+) h/g, '$1\\text{\\,h }')
                                      .replace(/(\d+) min/g, '$1\\text{\\,min }')
                                      .replace(/(\d+) s/g, '$1\\text{\\,s}')
      texteCorr += `${egalite} ${miseEnEvidence(reponseAvecUnites)}`
    } else {
