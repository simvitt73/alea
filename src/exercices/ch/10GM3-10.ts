...
export const titre = 'Convertir des durées entre formats heures-minutes-secondes, heures décimales et minutes décimales'
...
export default class ConvertirDuree extends Exercice {
  ...
  nouvelleVersion (): void {
    ...
    switch (this.sup) {
      case 1: typesDeQuestionsDisponibles = [1]; break // hhmmss → hdec
      case 2: typesDeQuestionsDisponibles = [2]; break // hhmmss → mindec
    ...
    for (let i = 0, cpt = 0, champIndex = 0; i < this.nbQuestions && cpt < 50; i++) {
      let texte: string
      ...
      if (uniteDepart === 'hhmmss') {
          ...
          const valeurBrute = typeof dureeDepart.mesure === 'number' ? dureeDepart.mesure : Number(dureeDepart.mesure)
      ...
      if (uniteArrivee === 'hhmmss') {
        ...
        if (uniteDepart !== 'hhmmss') {
          // Recalculer à partir de la valeur de départ arrondie
          const dureeDepArr = new Grandeur(valeurDepart as number, uniteDepart)
          const dureeArriveeArr = dureeDepArr.convertirEn(uniteArrivee)
          reponse = typeof dureeArriveeArr.mesure === 'number' ? dureeArriveeArr.mesure : Number(dureeArriveeArr.mesure)
        } else {
          const dureeArrivee = dureeBase.convertirEn(uniteArrivee)
          const reponseB = typeof dureeArrivee.mesure === 'number' ? dureeArrivee.mesure : Number(dureeArrivee.mesure)
    ...
    while((uniteArrivee==='hhmmss' && ((reponse as string).length === 3 || (reponse as string).length === 0))|| uniteDepart === 'hhmmss' && totalSecondes < 3600 || (uniteArrivee === 'hhmmss' && !((reponse as string).includes('h') && (reponse as string).includes('min') || (reponse as string).includes('h') && (reponse as string).includes('s') || (reponse as string).includes('min') && (reponse as string).includes('s')))) // Éviter les réponses du type "0 h 0 min 45 s" et s'assurer qu'au moins h et min ou h et s ou min et s apparaissent
      
      // Formater la question
      let valeurAffichee: string
      if (uniteDepart === 'hhmmss') {
        // valeurDepart est déjà au format "3 h 45 min 28 s", extraire les nombres
        const matches = (valeurDepart as string).match(/(\d+) h|(\d+) min|(\d+) s/g) || []
        const parts: string[] = []
        ...
        matches.forEach(match => {
          if (match.includes(' h')) {
            ...
            if (m > 0) parts.push(`${m}\\text{ ${m <= 1 ? 'minute' : 'minutes'}}`)
          } else if (match.includes(' s')) {
            const s = parseInt(match.replace(' s', ''))
            if (s > 0) parts.push(`${s}\\text{ ${s <= 1 ? 'seconde' : 'secondes'}}`)
          }
        })
      ...
      if (this.interactif) {
        if (uniteArrivee === 'hhmmss') {
          texte += '<br>' + ajouteChampTexteMathLive(this, champIndex, KeyboardType.clavierNumbers, { texteAvant: '', texteApres: ' h ' })
          texte += ajouteChampTexteMathLive(this, champIndex + 1, KeyboardType.clavierNumbers, { texteAvant: '', texteApres: ' min ' })
          texte += ajouteChampTexteMathLive(this, champIndex + 2, KeyboardType.clavierNumbers, { texteAvant: '', texteApres: ' s' })
        } else {
          texte += '<br>' + ajouteChampTexteMathLive(this, champIndex, KeyboardType.clavierNumbers, { texteAvant: ' ', texteApres: ` ${uniteAffichageArrivee}` })
        }
      }
      else {
        if (uniteArrivee === 'hhmmss') {
      ...
      texteCorr = ''
      const correctionDetaillee = this.genererCorrectionDetaillee(typeDeQuestion, valeurDepart, reponse, uniteDepart, uniteArrivee, estApproximation)
      if (this.correctionDetaillee) {
        texteCorr = correctionDetaillee.texte + '<br><br>'
      } 
      texteCorr += this.genererCorrectionSimple(valeurDepart, reponse, uniteDepart, uniteArrivee, correctionDetaillee.estApproximation)
      ...
      if (this.interactif) {
        if (uniteArrivee === 'hhmmss') {
          ...
          const parts = hmsString.match(/(\d+)\s*h|(\d+)\s*min|(\d+)\s*s/g) || []
  ...
  private genererCorrectionDetaillee(typeDeQuestion: number, valeurDepart: number | string, reponse: number | string, uniteDepart: string, uniteArrivee: string, estApproximation: boolean = false):  {texte: string, estApproximation: boolean} {
    let texteCorr = ''
    ...
    switch (typeDeQuestion) {
      case 1: // hhmmss → hdec
        ...
        const h1 = valStr1.match(/(\d+) h/) ? parseInt(valStr1.match(/(\d+) h/)![1]) : 0
        const m1 = valStr1.match(/(\d+) min/) ? parseInt(valStr1.match(/(\d+) min/)![1]) : 0
        const s1 = valStr1.match(/(\d+) s/) ? parseInt(valStr1.match(/(\d+) s/)![1]) : 0
        texteCorr = `Pour convertir un temps au format heures minutes secondes en heures, on convertit les minutes et secondes en fractions d'heures :<br><br>`
        if (h1 > 0) {
          texteCorr += ` heures : $${h1}\\text{\\,h}$ ;`
        }
        if (m1 > 0) {
          // Vérifier si m1/60 est périodique
          const minToHours = m1/60
          const isPeriodicMin = Math.round(minToHours * 100) !== minToHours * 100
          if (isPeriodicMin) estApproximation = true
          texteCorr += ` minutes : $${m1}\\text{\\,min} = \\dfrac{${m1}}{60}\\text{\\,h}$ ${s1 > 0 ? ' ;' : '.'};`
        }
        ...
        if (s1 > 0) {
          ...
          const secToHours = s1/3600
          const isPeriodicSec = Math.round(secToHours * 100) !== secToHours * 100
          if (isPeriodicSec) estApproximation = true
          texteCorr += ` secondes : $${s1}\\text{\\,s} = \\dfrac{${s1}}{3600}\\text{\\,h} ${isExactSec?`=${secToHours}\\text{\\,h}`: ''}$.`
        }
        texteCorr += `<br>On somme : $`
        const parts = []
        ...
        if (estApproximation) {
          texteCorr += ` \\approx ${texNombre(reponse as number, 2)}\\text{\\,h}$`
        } else {
          texteCorr += ` = ${texNombre(reponse as number, 2)}\\text{\\,h}$`
        }
        break
      ...
      case 2: // hhmmss → mindec
      {
        ...
        const h2 = valStr2.match(/(\d+) h/) ? parseInt(valStr2.match(/(\d+) h/)![1]) : 0
        const m2 = valStr2.match(/(\d+) min/) ? parseInt(valStr2.match(/(\d+) min/)![1]) : 0
        const s2 = valStr2.match(/(\d+) s/) ? parseInt(valStr2.match(/(\d+) s/)![1]) : 0
        ...
        if (s2 > 0) {
          ...
          const secToMin = s2/60
          const isPeriodicSec2 = Math.round(secToMin * 1000) !== secToMin * 1000
      ...
      case 3: // hdec → hhmmss
        ...
        const hFinal = reponseString.match(/(\d+) h/) ? parseInt(reponseString.match(/(\d+) h/)![1]) : 0
        const mFinal = reponseString.match(/(\d+) min/) ? parseInt(reponseString.match(/(\d+) min/)![1]) : 0
        const sFinal = reponseString.match(/(\d+) s/) ? parseInt(reponseString.match(/(\d+) s/)![1]) : 0
      ...
      case 6: // mindec → hdec
        ...
        if (texNombre(reponse as number, 2) !== texNombre(reponse as number, 3)) estApproximation = true
  ...
  private genererCorrectionSimple(valeurDepart: number | string, reponse: number | string, uniteDepart: string, uniteArrivee: string, estApproximation: boolean): string {
    let texteCorr = '$'
    ...
    if (typeof valeurDepart === 'string') {
      ...
      const valeurAvecUnites = valeurDepart.replace(/(\d+) h/g, '$1\\text{\\,h }')
    ...
    if (typeof reponse === 'string') {
      ...
      let reponseAvecUnites = reponse.replace(/(\d+) h/g, '$1\\text{\\,h }')
                                    .replace(/(\d+) min/g, '$1\\text{\\,min }')
                                    .replace(/(\d+) s/g, '$1\\text{\\,s}')
      
      // Ajouter "0 s" si il y a des heures ou minutes mais pas de secondes
      if ((reponse.includes('h') || reponse.includes('min')) && !reponse.includes('s')) {
