...
export default class ExerciceAdditionnerSoustraireFractions5ebis extends Exercice {
  ...
  constructor () {
    ...
    this.besoinFormulaire5Numerique = ['Pourcentage de fractions avec numérateur négatif', 100]
    this.level = 5
  }
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, a, b, c, d, k, s, ordreDesFractions, texte, texteCorr; i < this.nbQuestions;) {
      ...
      if (listeTypeDeQuestions[i] === '-') {
        c = choice([randint(1, b * k), randint(b * k, 9 * k)]) * (cNegatif ? -1 : 1)
      } else {
      ...
      if (listeTypeDeQuestions[i] === '+') { // une addition
        if (!context.isAmc || (this.interactif && this.interactifType === 'qcm')) {
          ...
          this.autoCorrection[i].propositions = [
            {
              texte: this.sup3 ? `$${new FractionEtendue(a * k + c, d).texFractionSimplifiee}$` : `$${new FractionEtendue(a * k + c, d).texFraction}$`,
              statut: true
            },
            {
              texte: this.sup3 ? `$${new FractionEtendue(a + c, d).texFractionSimplifiee}$` : `$${new FractionEtendue(a + c, d).texFraction}$`,
              statut: false
            },
            {
              texte: this.sup3 ? `$${new FractionEtendue(a + c, b + d).texFractionSimplifiee}$` : `$${new FractionEtendue(a + c, b + d).texFraction}$`,
              statut: false
            },
            {
              texte: this.sup3 ? `$${new FractionEtendue(a * c, d).texFractionSimplifiee}$` : `$${new FractionEtendue(a * c, d).texFraction}$`,
          ...
          if (this.level === 6) {
            ...
            // @ts-expect-error
            this.autoCorrection[i].propositions[0].texte = `$${new FractionEtendue(a + c, b).texFraction}$`
          }
          /*************************************************************************/
        }
        ...
        texteCorr += a * k + c < 0 ? `=${new FractionEtendue(a * k + c, d).texFSD}$` : '$'
        ...
        if (!context.isAmc) {
          if (this.interactif && this.interactifType === 'qcm') {
            ...
            handleAnswers(this, i, { reponse: { value: new FractionEtendue(a * k + c, d).toLatex(), options: { fractionIrreductible: this.sup3, fractionEgale: !this.sup3 } } })
          }
        }
        ...
        if (!context.isAmc || (this.interactif && this.interactifType === 'qcm')) {
          this.autoCorrection[i].propositions = [
            ...
            {
              texte: this.sup3 ? `$${new FractionEtendue(a - c, Math.abs(b - d)).texFractionSimplifiee}$` : `$${new FractionEtendue(a - c, Math.abs(b - d)).texFraction}$`,
              statut: false
            },
            {
              texte: this.sup3 ? `$${new FractionEtendue(a - c, d).texFractionSimplifiee}$` : `$${new FractionEtendue(a - c, d).texFraction}$`,
