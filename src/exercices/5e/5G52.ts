...
export default class nomExercice extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      const rLongueurPatron = placeLatexSurSegment(`${listeTypeQuestions[i] === 'CylindreVersPatron' ? '\\approx' : ''}\\text{${texNombre(perimetre, 1)} cm}`, ALongueurPatron, BLongueurPatron, { distance: 1 })
      ...
      if (choice([0, 1]) === 0) {
        ...
        rHauteurPatron = placeLatexSurSegment(`\\text{${hEx} cm}`, A, B, { distance: 1 })
      } else {
        ...
        rHauteurPatron = placeLatexSurSegment(`\\text{${hEx} cm}`, C, D, { distance: 1 })
      }
      if (choice([0, 1]) === 0) {
      ...
      let rRayon = placeLatexSurSegment(`\\text{${rEx} cm}`, ptExtremite1Rayon, ptExtremite2Rayon)
      let qDiametre = placeLatexSurSegment('\\text{.......}', ptExtremite1Diametre, ptExtremite2Diametre)
      let rDiametre = placeLatexSurSegment(`\\text{${2 * rEx} cm}`, ptExtremite1Diametre, ptExtremite2Diametre)
      if (listeTypeQuestions[i] === 'CylindreVersPatron') {
        qRayon = placeLatexSurSegment('\\text{.......}', ptExtremite1Rayon, ptExtremite2Rayon)
        rRayon = placeLatexSurSegment(`\\text{${rEx} cm}`, ptExtremite1Rayon, ptExtremite2Rayon)
        qDiametre = placeLatexSurSegment('\\text{.......}', ptExtremite1Diametre, ptExtremite2Diametre)
        rDiametre = placeLatexSurSegment(`\\text{${2 * rEx} cm}`, ptExtremite1Diametre, ptExtremite2Diametre)
