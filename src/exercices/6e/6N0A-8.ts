...
function generateNumbers (
  ...
  do {
    ...
    if (decimales.length !== 6) { // Cas où on n'a pas choisi les trois ensemble : dixième, centième et millième
      const decimaleAAjouter = shuffle(range1(9))
      if (niveaux.length === 1) { // Cas où on a choisi qu'en seul des trois parmi : dixième, centième et millième
        if (niveaux[0] === 1) { // Que des dixièmes
          let d3: number
          for (let indiceTableau = 1; indiceTableau < 6; indiceTableau++) {
            d3 = (c3 + decimaleAAjouter[indiceTableau]) % 10
            decimales.push([d3])
          }
        } else if (niveaux[0] === 2) { // Que des centièmes
          decimales.push([c4, c3])
          decimales.push([0, c3])
          decimales.push([c4, 0])
          decimales.push([randint(1, 9, [c3]), c4])
          decimales.push([randint(1, 9, [c4, 0]), c3])
        } else { // Que des millièmes
          decimales.push([c3, c4, (c5 + 1) % 10])
          decimales.push([(c3 + 1) % 10, c4, c5])
        }
      } else { // Cas où on a choisi que deux parmi : dixième, centième et millième
        if (!niveaux.includes(3)) { // Pas de millième
          decimales.push([c4])
          decimales.push([c4, c3])
          decimales.push([0, c4])
          decimales.push([0, c3])
        } else if (!niveaux.includes(2)) { // Pas de centième
          decimales.push([c4])
          decimales.push([(c3 + 1) % 10, c4, c5])
        } else { // Pas de dixième
          decimales.push([c4, c3])
    ...
    for (let indiceTableau = 0; indiceTableau < 6; indiceTableau++) {
      ...
      const entier = proximite
        ? baseEntier
        : (numbers.length % 2 === 0 ? baseEntier : alternateEntier)
...
function getDecimalFraction (n: number, digits: number): { num: number; den: number } {
...
function formatterPartieEntiere (nombres: number[]): string {
  ...
  if (len === 2) return `$${miseEnEvidence(uniques[0], bleuMathalea)}$ et $${miseEnEvidence(uniques[1], bleuMathalea)}$`
...
export default class RangerOrdreCroissantDecroissant extends Exercice {
  ...
  nouvelleVersion () {
    ...
    let typesDeNombresEntiers = typesDeNombres.map(value => parseInt(value.toString(), 10))
    ...
    for (
      ...
      } else { // EE : Cette partie sur les entiers date d'un ancien exo et j'ai choisi de laisser ainsi.
        const c1 = randint(1, 9)
        const c2 = randint(1, 9, [c1])
        const c3 = randint(1, 9, [c1, c2])
        const c4 = randint(1, 9, [c1, c2, c3])
      ...
      if (this.correctionDetaillee && this.sup3) {
        texteCorr += 'Pour ranger les nombres dans l’ordre croissant, on procède en deux temps.<br>'

        // Étape 1 : Classement des parties entières
        texteCorr += texteEnCouleurEtGras('Comparons les parties entières', bleuMathalea) + ' :<br>'
        if ([...new Set(nombres.map(n => Math.floor(n)))].length === 1) { // Recherche des parties entières des nombres à classer
          ...
          const partiesEntieres = [...new Set(nombres.map(n => Math.floor(n)))].sort((a, b) =>
        ...
        texteCorr += texteEnCouleurEtGras('Comparons les parties décimales lorsque les parties entières sont identiques', bleuMathalea) + ' :<br>'
        ...
        let numeroGroupe = -1
        for (const entier of Object.keys(groupesParEntier).map(Number).sort((a, b) => a - b)) {
          const groupe = groupesParEntier[entier]
          if (groupe.length > 1) {
            numeroGroupe++
            if (!this.sup4) texteCorr += numAlpha(numeroGroupe) + ` Parmi les nombres ayant pour partie entière $${miseEnEvidence(entier, bleuMathalea)}$, on va comparer les parties décimales :<br>`
            const numerateurs = []
            ...
            texteCorr += '  → On classe ensuite les nombres en comparant leurs fractions décimales respectives'
            texteCorr += this.sup4
              ? '.<br>'
      ...
      handleAnswers(this, i,
        {
          bareme: (listePoints) => [Math.ceil((listePoints[0] * listePoints[1] + listePoints[2] + listePoints[3] + listePoints[4] + listePoints[5]) / 2), 3],
