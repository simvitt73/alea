...
export default class ConstruireParSymetrie extends Exercice {
  ...
  _choisi3Points (d: Droite, lieu = ['dessus', 'dessous', 'sur']) {
    ...
    do { // on vérifie que les points sont assez espacés les uns des autres.
      ...
      lBC = longueur(pB, pC)
      count3++
    } while ((lAB < 2 || lAC < 2 || lBC < 2 || Number(aireTriangle(polygone(A, B, C))) < 15) && count3 < 20)
  ...
  nouvelleVersion () {
    ...
    for (
      ...
      if (listeTypeDeQuestions[i] < 6) {
        positionLabelDroite = translation(pointSurDroite(d, d.b === 0 ? Ymin + 1 : Xmin + 1, ''), homothetie(vecteur(d.a, d.b), A, 0.5 / norme(vecteur(d.a, d.b))) as Vecteur)
        objetsEnonce.push(texteParPoint('(d)', positionLabelDroite, 0, 'black', 1.5, 'milieu', true))
        objetsCorrection.push(texteParPoint('(d)', positionLabelDroite, 0, 'black', 1.5, 'milieu', true))
      }
      const params = {
        xmin: Xmin,
        ymin: Ymin,
