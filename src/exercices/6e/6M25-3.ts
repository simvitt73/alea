...
export default class FormulesAireCarreRectangle extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      if (this.questionJamaisPosee(i, listeTypeDeQuestions[i])) { // <- laisser le i et ajouter toutes les variables qui rendent les exercices différents (par exemple a, b, c et d)
        let texte = ''
        let texteCorr = ''
        const texteFixe = []
  ...
  correctionInteractive = (i: number) => {
    ...
    if (select1?.value != null && select2.value != null && select3.value != null) {
      ...
      if (this.typeDeQuestions[i] === 'mots') {
        ...
        if (!isOk1) {
          ...
          feedback = 'Cette aire est bien un produit mais pas des éléments choisis.'
        }
      } else { // operation
    ...
    if (i < 6) { // Pour éviter la création d'un champ impossible nb de questions > 6
