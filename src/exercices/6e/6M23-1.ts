    ...
    for (let i = 0, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) { // Boucle principale où i+1 correspond au numéro de la question
      ...
      if (this.sup2) {
        ...
        val = choice([
          ...
          arrondi(randint(1, 9) * 100 + randint(1, 9) * 10 + (randint(1, 9)) / 100)
        ])
        // XX,X 0,X 0,0X X,XX
      } else {
        val = choice([
      ...
      const facteur = versUnitePlusPetite ? '100' : new FractionEtendue(1, 100).texFraction
      const reponse = texNombre(versUnitePlusPetite ? arrondi(val * 100) : arrondi(val / 100))
      ...
      if (!versUnitePlusPetite)texteCorr += `$${texNombre(val)} ${miseEnEvidence(`\\times \\dfrac{1}{100}${sp()}${unite2.replaceAll('$', '')}`, bleuMathalea)}$ = `
