...
export default class ProduitDeDecimauxAPartirProduitConnu extends Exercice {
  constructor () {
    ...
    this.comment = 'Dans le premier paramètre, nb1, nb2 représentent des entiers entre 2 et 99.<br> p1 et p2 représentent des entiers non nuls entre -3 et 3.'
    this.besoinFormulaire2Numerique = ['Nombre de chiffres non nuls dans le plus petit facteur', 4, '1 : Un seul chiffre\n2 : Deux chiffres\n3 : Mélange']
    ...
    for (let i = 0, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      if (this.sup3) {
        ...
        texte += this.interactif
          ? ajouteChampTexteMathLive(this, i, KeyboardType.clavierNumbers, { texteAvant: ' : ' })
          : '.'
        handleAnswers(this, i, { reponse: { value: reponse[1], options: { nombreDecimalSeulement: true } } })
        texteCorr = ''
        reponse[0] = 0
      } else {
        texte = numAlpha(0) + `Calculer en ligne $${nb1}\\times ${nb2}$`
        texte += this.interactif
          ? ajouteChampTexteMathLive(this, 2 * i, KeyboardType.clavierNumbers, { texteAvant: ' puis donner le résultat : ', texteApres: '.<br>' })
          : '.<br>'
        handleAnswers(this, 2 * i, { reponse: { value: reponse[0], options: { nombreDecimalSeulement: true } } })

        texte += numAlpha(1) + `En déduire le résultat de $${texNombre(multipleNb1)}\\times ${texNombre(multipleNb2)}$`
        texte += this.interactif
          ? ajouteChampTexteMathLive(this, 2 * i + 1, KeyboardType.clavierNumbers, { texteAvant: ' : ', texteApres: '.' })
          : '.'
        handleAnswers(this, 2 * i + 1, { reponse: { value: reponse[1], options: { nombreDecimalSeulement: true } } })
      ...
      (p1 === 0 ? '' : `${nb1}\\times ${nb2}$ ${nomUnitePourPuissance(p1 + p2)} $ = `) + // si p1=0, alors cette ligne est équivalente à la précédente
      `${texNombre(nb1 * nb2)}$ ${nomUnitePourPuissance(p1 + p2)} $ =
      ${miseEnEvidence(texNombre(reponse[1]))}$`)

      if (this.questionJamaisPosee(i, reponse[1])) { // Si la question n'a jamais été posée, on en crée une autre
