...
export default class VocabulaireEtOperations extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (
      ...
      const operation = operationPresente != null ? operationPresente[0] : 'undefined'
      ...
      const motCle = laBonneParole != null && laBonneParole[1] != null ? laBonneParole[1] : 'undefined' // Pour traquer les erreurs
      const phrases = [
        'La somme de § et de £',
      ...
      switch (listeTypeDeQuestions[i]) {
        ...
        case 2: // proposer une expression en français
          if (String(expn).indexOf('ou') > 0) { expn = String(expn).substring(0, String(expn).indexOf('ou')) } // on supprime la deuxième expression fractionnaire
          texte += numAlpha(i) + (this.interactif
