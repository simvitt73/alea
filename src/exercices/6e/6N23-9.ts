...
export default class NombreDecimalOraliseDeDifferentesManieres extends Exercice {
  ...
  '3 : Mélange'
    ...
    this.sup4 = this.besoinFormulaire4CaseACocher = ['Que depuis l\'unité', false]
  }
  ...
  nouvelleVersion () {
    ...
    const listeTypeDeQuestions3 = listeTypeDeQuestions.map(value => (parseInt(value.toString(), 10) - 1))
    if (this.sup4) listeTypeDeQuestions3.push(3)

    enleveDoublonNum(listeTypeDeQuestions3)
    if (listeTypeDeQuestions3.length === 1) listeTypeDeQuestions3.push(randint(0, 6, listeTypeDeQuestions3[0]))

    const binomesConversion = this.sup4
      ? this.sup2 === '1'
        ? genererBinomesAleatoires(listeTypeDeQuestions3, { premierNombre: 3, ordreCroissant: true })
        : this.sup2 === '2'
          ? genererBinomesAleatoires(listeTypeDeQuestions3, { premierNombre: 3, ordreDecroissant: true })
          : genererBinomesAleatoires(listeTypeDeQuestions3, { premierNombre: 3 })
      : this.sup2 === '1'
        ? genererBinomesAleatoires(listeTypeDeQuestions3, { ordreCroissant: true })
        : this.sup2 === '2'
          ? genererBinomesAleatoires(listeTypeDeQuestions3, { ordreDecroissant: true })
          : genererBinomesAleatoires(listeTypeDeQuestions3)

    const unites = ['milliers', 'centaines', 'dizaines', 'unités', 'dixièmes', 'centièmes', 'millièmes']
    const unitesNumeriques = [texNombre(1000), 100, 10, 1, new FractionEtendue(1, 10).texFraction, new FractionEtendue(1, 100).texFraction, new FractionEtendue(1, 1000).texFraction]
    const denominateur = [1, 1, 1, 1, 10, 100, 1000]
    for (let i = 0, texte, texteCorr, cpt = 0; i < Math.min(this.nbQuestions, binomesConversion.length) && cpt < 50;) {
      const reponse = Math.pow(10, binomesConversion[i][1] - binomesConversion[i][0])
      if (this.sup3) {
        ...
        texte += this.interactif
          ? ajouteChampTexteMathLive(this, i, KeyboardType.clavierNumbers, { texteApres: unites[binomesConversion[i][1]] })
        ...
        texte += this.interactif
          ? binomesConversion[i][1] > 3
            ? remplisLesBlancs(this, i, `$\\dfrac{%{champ1}}{${texNombre(denominateur[binomesConversion[i][1]])}}`, KeyboardType.clavierNumbers, '\\ldots\\ldots')
            : ajouteChampTexteMathLive(this, i, KeyboardType.clavierNumbers, { texteApres })
