...
const choisitTriangle = function (typeAxe: number, index: number) { // retourne {antecedent: number, image: number, distracteurs: [number]}
  let figA
  let antecedent
...
export default class SymetrieAxialePavageTriangulaire extends Exercice {
  ...
  nouvelleVersion (numeroExercice: number) {
    ...
    for (let i = 0; i < this.nbQuestions; i++) {
      ...
      images[i] = symetrieAxiale(triAngles[question[i].antecedent].tri, d[i]) as Polygone
