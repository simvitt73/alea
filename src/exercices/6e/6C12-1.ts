...
export default class QuestionsMasses extends Exercice {
  ...
  nouvelleVersion () {
    for (let i = 0, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      (typesQuestionsDisponibles.length === 1 && this.nbQuestions === 1) ? this.consigne = 'Répondre à la question suivante.' : this.consigne = 'Répondre aux questions suivantes.'
      let PrixUnitaire
      let AutrePrix
      let PrixReduction
      if (this.sup3) {
      ...
      for (let kk = 0; kk < typesQuestionsDisponibles.length; kk++) {
        if (typesQuestionsDisponibles.length > 1) { // Si une seule question, pas besoin de puces numerotees
