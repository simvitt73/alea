...
//
  ...
  nouvelleVersion (numeroExercice: number) {
    for (let i = 0, colonne1, colonne2, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      const options = { eleId: numeroExercice + '_' + i, widthmincol1: '500px', widthmincol2: '300px', largeur1: 60 }
      const enonce = deuxColonnesResp(colonne1, colonne2, options)
      const optionsSol = { eleId: 's-' + numeroExercice + '_' + i, widthmincol1: '500px', widthmincol2: '300px', largeur1: 60 }
      ...
      // Si la question n'a jamais été posée, on en crée une autre
        ...
        const reportWindowSize = function () {
          ...
          if (element !== null &&
            ...
            const widthMathalea2d = parseInt(qcms[0].getAttribute('width') ?? '800')
            ...
            const diff = (element.parentElement != null ? element.parentElement.clientWidth : 1000) - parseInt(options.widthmincol1.replaceAll('px', ''))
            element.style.minWidth = options.widthmincol1
            element3.style.minWidth = options.widthmincol1
            if (element.parentElement != null && element3.parentElement != null) {
              if (diff > col2) {
