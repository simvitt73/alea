...
export default class AngleProduitScalaire extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, texte, texteCorr, xA, yA, xB, yB, xC, yC, A, B, C, r, o, objets, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      objets = []
      xA = randint(-2, 2)
      yA = randint(-2, 2)
      xB = randint(-5, 5, xA)
      ...
      do {
        ...
        xC = randint(-5, 5, [xA, xB])
        yC = randint(-5, 5)
      } while ((xB - xA) * (yC - yA) - (yB - yA) * (xC - xA) === 0 || (xB - xA) * (xC - xA) + (yB - yA) * (yC - yA) === 0)
      A = point(xA, yA)
      B = point(xB, yB)
      C = point(xC, yC)
      const demiDroiteAB = demiDroite(A, B, 'blue')
      const demiDroiteAC = demiDroite(A, C, 'blue')
      const labelA = latex2d('A', xA - 0.5, yA + 0.5, { letterSize: 'normalsize' })
      const labelB = latex2d('B', xB - 0.5, yB + 0.5, { letterSize: 'normalsize' })
      const labelC = latex2d('C', xC - 0.5, yC - 0.5, { letterSize: 'normalsize' })
      ...
      const redABAC = extraireRacineCarree(((xC - xA) ** 2 + (yC - yA) ** 2) * ((xB - xA) ** 2 + (yB - yA) ** 2))
...
\\end{aligned}$`
      ...
      objets.push(r, o, traceABetC, labelA, demiDroiteAB, demiDroiteAC, labelB, labelC)
      switch (listeTypeDeQuestions[i]) { // listeTypeDeQuestions[i]
        case 1: //

          texte = texteIntro +
         createList({
           items: ['Calculer $\\overrightarrow{AB}\\cdot\\overrightarrow{AC}$ en utilisant les coordonnées.',
        ...
        case 2: //
          texte = texteIntro + '<br>'
          texte += 'Déterminer la valeur approchée en degré  à 0,1 près de l\'angle $\\widehat{BAC}$.'
          ...
          texteCorr += `${texteGras('Calcul du produit scalaire avec les coordonnées  :<br>')}` + texteCorrQ1 + '<br><br>'
          texteCorr += `${texteGras('Calcul du produit scalaire avec le cosinus  :<br>')}` + texteCorrQ2 + '<br><br>'
        ...
        default :
          texte = texteIntro + '<br>'
          texte += 'Déterminer la valeur approchée en degré  à 0,1 près de l\'angle $\\widehat{BAC}$.'
          texte += mathalea2d(Object.assign({ zoom: 1, scale: 0.6 }, fixeBordures(objets)), objets)
          texteCorr = `${texteGras('Méthode :')} on calcule le produit scalaire de deux manières, d'abord avec les coordonnées puis avec le cosinus.<br>
          En utilisant ces deux expressions, on en déduit la mesure de l'angle $\\widehat{BAC}$. <br><br>`
          texteCorr += `${texteGras('Calcul du produit scalaire avec les coordonnées  :<br>')}` + texteCorrQ1 + '<br><br>'
          texteCorr += `${texteGras('Calcul du produit scalaire avec le cosinus  :<br>')}` + texteCorrQ2 + '<br><br>'
      ...
      handleAnswers(this, i, { reponse: { value: texNombre(arrondi(Math.acos(((xB - xA) * (xC - xA) + (yB - yA) * (yC - yA)) / (Math.sqrt(((xC - xA) ** 2 + (yC - yA) ** 2) * ((xB - xA) ** 2 + (yB - yA) ** 2)))) * 180 / Math.PI, 1)) } })
      if (this.interactif) {
        texte += '<br>' + ajouteChampTexteMathLive(this, i, ' ', { texteAvant: '$\\widehat{BAC}\\approx$', texteApres: '$^{\\circ}$' })
      }
