...
export default class ProduitScalaireAlKashi extends Exercice {
  ...
  nouvelleVersion () {
    for (let i = 0, texte, texteCorr, reponse, objets, objetsC, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      objets = []
      objetsC = []
      ...
      do {
        ...
        longueurBC = randint(5, Math.min(longueurAB + longueurAC - 2, 10), [longueurAB, longueurAC]) + randint(1, 89) / 100
        A = point(0, 0)
      ...
      const codeAB = angleOriente(C, A, B) > 0 ? texteSurSegment(`$${texNombre(longueurAB, 1)}$`, A, B, 'black', 0.5, true) : texteSurSegment(`$${texNombre(longueurAB, 1)}$`, B, A, 'black', 0.5, true)
      const codeAC = angleOriente(B, C, A) > 0 ? texteSurSegment(`$${texNombre(longueurAC, 1)}$`, C, A, 'black', 0.5, true) : texteSurSegment(`$${texNombre(longueurAC, 1)}$`, A, C, 'black', 0.5, true)
      const codeBCcours = angleOriente(A, B, C) > 0 ? texteSurSegment('$a$', B, C, 'black', 0.5, true) : texteSurSegment('$a$', C, B, 'black', 0.5, true)
      const codeABcours = angleOriente(C, A, B) > 0 ? texteSurSegment('$b$', A, B, 'black', 0.5, true) : texteSurSegment('$b$', B, A, 'black', 0.5, true)
      const codeACcours = angleOriente(B, C, A) > 0 ? texteSurSegment('$c$', C, A, 'black', 0.5, true) : texteSurSegment('$c$', A, C, 'black', 0.5, true)
      ...
      objetsC.push(ABC, nommeABCcours, codeAngleA, codeAngleB, codeAngleC, codeACcours, codeABcours, codeBCcours)
      ...
      texteCorr += ` Dans le triangle  $${nom[0]}${nom[1]}${nom[2]}$, on connaît les longueurs $${nom[0]}${nom[1]}$, $${nom[0]}${nom[2]}$ et l'angle $\\widehat{${nom[1]}${nom[0]}${nom[2]}}$.<br>
          On applique la formule d'Al Kashi  : <br><br>
              ...
              \\end{aligned}$`
      reponse = texNombre(Math.sqrt(longueurAB ** 2 + longueurAC ** 2 - 2 * longueurAB * longueurAC * Math.cos(arrondi(angle(B, A, C) * pi / 180, 4))), 1)
      handleAnswers(this, i, { reponse: { value: reponse } })
      texte += ajouteChampTexteMathLive(this, i, KeyboardType.clavierDeBase, { texteAvant: `<br>$${nom[1]}${nom[2]}\\approx$` }) + '<br>'
