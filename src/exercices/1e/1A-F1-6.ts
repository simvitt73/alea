...
versionAleatoire = () => {
    ...
    let resultat = aXCarre.sommeFraction(bX).sommeFraction(cFraction)
    ...
    const aXCarreCommun = new FractionEtendue(aXCarre.num * (denominateurCommun / aXCarre.den), denominateurCommun)
    const bXCommun = new FractionEtendue(bX.num * (denominateurCommun / bX.den), denominateurCommun)
    const cFractionCommun = new FractionEtendue(c * (denominateurCommun / cFraction.den), denominateurCommun)
    
    // Calcul du numérateur final
    const numerateurFinal = aXCarreCommun.num + bXCommun.num + cFractionCommun.num
    ...
    const distracteur2 = aXCarre.sommeFraction(bXErreur).sommeFraction(cFraction)
    
    // Distracteur 3 : erreur dans le calcul de x² (l'élève ne met pas le signe au carré)
    const xCarreErreur = new FractionEtendue(-frac[0] * frac[0], frac[1] * frac[1])
    const aXCarreErreur = xCarreErreur.produitFraction(new FractionEtendue(a,1))
    const distracteur3 = aXCarreErreur.sommeFraction(bX).sommeFraction(cFraction)
