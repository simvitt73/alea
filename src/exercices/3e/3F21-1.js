...
export default function LectureExpressionFonctionsAffines () {
  ...
  this.nouvelleVersion = function (numeroExercice) {
    ...
    for (let i = 0; i < nbDroites; i++) {
      this.listeQuestions.push(`Déterminer l'expression de la fonction $f_${i + 1}$ représentée par la droite $(d_${i + 1})$.${this.interactif ? '<br>' : ''}` + ajouteChampTexteMathLive(this, i, ' ', { texteAvant: sp() + `$f_${i + 1}(x)=$` }))
