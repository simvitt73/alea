...
export const titre = 'Préparation DNB : Volume, fonctions, vitesse, grandeurs composées'
...
export default class ExercicePolynesie52024 extends ExerciceBrevetA {
  ...
  private appliquerLesValeurs (r: number, h:number, v2:number, nom: string, duree:number, reste1: number, reste2: number) : void {
    const v1 = v2 * 60
    ...
    const rep = new RepereBuilder({ xMin, xMax, yMax, yMin })
      ...
      .setGrille({ grilleX: { xMin, xMax, dx: 5 }, grilleY: { yMin, yMax, dy: 5 } })
      .setGrilleSecondaire({ grilleX: { xMin, xMax, dx: 0.5 }, grilleY: { yMin, yMax, dy: 0.5 } })
    rep.yLabelEcart = 0.8
    ...
    const question3a = 'Montrer que l\'expression $v(t)$ permet de déterminer le volume d\'eau en m$^3$ qu\'il reste à vider dans le bassin en fonction de la durée $t$, exprimée en minutes, d\'utilisation de la pompe.'
...
Répondre aux questions suivantes par une lecture graphique.`
    ...
    const question4b = 'Déterminer le temps nécessaire pour que la pompe vide complètement le bassin.'
...
Au bout de $${miseEnEvidence(`${Math.round((volumeArrondi - reste2) / v2 / 5) * 5}~\\text{minutes}`)}$, il reste donc $${texNombre(reste2, 1)}~\\text{m}^3$ à vider.`
  ...
  versionAleatoire: () => void = () => {
    const nom = choice(['de la pinède', 'du petit dauphin', 'des sables', 'du petit vent', 'des hirondelles', 'des alizés', 'des chênes', 'des palmiers', 'du lac'])
    let r: number
    let h: number
    let v2: number
    let duree: number
