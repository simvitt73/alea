...
export default class ExerciceMetropole392024 extends ExerciceBrevetA {
  constructor () {
    ...
    this.sup = false
    this.introduction = texteItalique('D\'après l\'exercice 1 du brevet Métropole juin 2025.<br>')

    this.versionAleatoire()
  }

  private appliquerLesValeurs (urneA: number[], urneB:number[], table: number, min: number, ajout: number) : void {
    const nbPrem = urneB.filter(n => estPremier(n)).length
    const probaPrem = new FractionEtendue(nbPrem, urneB.length)
    const probaPair = new FractionEtendue(urneA.filter(n => n % 2 === 0).length, urneA.length)
    const probaSupA = new FractionEtendue(urneA.filter(n => n >= min).length, urneA.length)
    const probaSupB = new FractionEtendue(urneB.filter(n => n >= min).length, urneB.length)
    const probaSupAajout = new FractionEtendue([...urneA, ajout].filter(n => n >= 20).length, urneA.length + +1)
    const probaSupBajout = new FractionEtendue([...urneB, ajout].filter(n => n >= 20).length, urneB.length + 1)
...
Les boules sont indiscernables au toucher.<br>`
    const question1 = 'On tire une boule dans l\'urne A, quelle est la probabilité d\'obtenir un nombre pair ?'
    ...
    const correction1 = `Les nombres pairs présents dans l'urne A sont : ${urneA.filter(n => n % 2 === 0).map(n => `$${n}$`).join(' ; ')}.<br>
  Il y a donc $${urneA.filter(n => n % 2 === 0).length}$ nombres pairs sur $${urneA.length}$. La probabilité d'obtenir un nombre pair est de $${probaPair.texFraction}${probaPair.estIrreductible ? '' : '=' + probaPair.texFractionSimplifiee}$.`
    const correction2 = `Les nombres premiers présents dans l'urne B sont : ${urneB.filter(n => estPremier(n)).map(n => `$${n}$`).join(' ; ')}.<br>
  Il y a donc $${nbPrem}$ nombres premiers sur $${urneB.length}$. La probabilité d'obtenir un nombre premier est de $${probaPrem.texFraction}${probaPrem.estIrreductible ? '' : '=' + probaPrem.texFractionSimplifiee}$.`
    const correction3 = `L'urne A contient ${nbMulA} boules dont le numéro est un multiple de $${table}$ (${urneA.filter(n => n % table === 0).map(el => `$\\,${el}\\,$`).join(';')}) et l'urne B en contient ${nbMulB} (${urneB.filter(n => n % table === 0).map(el => `$\\,${el}\\,$`).join(';')}).<br>
  L'urne ${nbMulA > nbMulB
  ...
  versionOriginale: () => void = () => {
    this.appliquerLesValeurs([7, 10, 12, 15, 24, 30], [2, 5, 6, 8, 17, 18, 21, 22, 25], 6, 20, 50)
  }
  ...
  versionAleatoire: () => void = () => {
    ...
    do {
      ...
      probaPair = new FractionEtendue(urneA.filter(n => n % 2 === 0).length, urneA.length)
      probaMul1 = new FractionEtendue(urneA.filter(n => n % mul === 0).length, urneA.length)
      probaMul2 = new FractionEtendue(urneB.filter(n => n % mul === 0).length, urneB.length)
      probaSupA = new FractionEtendue(urneA.filter(n => n >= min).length, urneA.length)
      probaSupB = new FractionEtendue(urneB.filter(n => n >= min).length, urneB.length)
    } while (probaMul1.inferieurstrict(2 / urneA.length) ||
