...
class Syracuse {
  ...
  constructor (Num: number) {
    ...
    this.coordonneesSuiteDeSyracuse = this.fctCoordonneesSuiteDeSyracuse(this.suiteDeSyracuse)
    ...
    this.tempsDeVolEnAltitude = this.fctTempsDeVolEnAltitude(this.suiteDeSyracuse)
  }
...
export default class ConjectureDeSyracuse extends Exercice {
  ...
  nouvelleVersion () {
    ...
    while (uneSuite.tempsDeVol > 25 || uneSuite.tempsDeVol < 5 || uneSuite.altitudeMaximale > 100) {
    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      const B = point(uneSuite.tempsDeVol / xCoeff, uneSuite.suiteDeSyracuse[0] / yCoeff)
      const s = segment(A, B, 'red')
      // let t = texteParPoint('mon texte',B);
      const t = texteParPosition(
        'Altitude initiale',
        uneSuite.tempsDeVol / xCoeff,
        uneSuite.suiteDeSyracuse[0] / yCoeff + 0.2, 0,
        'red', 1,
        'milieu',
        true
      )
