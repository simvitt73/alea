...
export default class ExercicePolynesie392024 extends ExerciceBrevetA {
  constructor () {
    ...
    this.sup = false
    this.introduction = texteItalique('D\'après l\'exercice 5 du brevet centres étrangers 2024.<br>')

    this.versionAleatoire()
  }

  private appliquerLesValeurs (r: number, h: number, tt: number, rapport: FractionEtendue, pourcentage: number, quidam: string) : void {
    ...
    const figure1 = mathalea2d(Object.assign({ pixelsParCm: 30, scale: 1 }, fixeBordures(objets)), objets)
    ...
    const latexOM = placeLatexSurSegment(`${texNombre(SM, 1)}\\text{~cm}`, M1, S2, { distance: 0.3, letterSize: 'footnotesize' })
    const longArc = latex2d(`${texNombre(circo1, 1)}\\text{~cm}`, 0, 4, { letterSize: 'footnotesize' })
    const objets2 = [secteur, labels2, latexOM, longArc]
    const figure2 = mathalea2d(Object.assign({ pixelsParCm: 30, scale: 1 }, fixeBordures(objets2)), objets2)
...
\\end{array}$`
    ...
    const cone2 = cone({ centre: O2, rx: 2 * rapport.num / rapport.den, hauteur: -6 * rapport.num / rapport.den, couleurDeRemplissage: 'lightgray', opaciteDeRemplissage: 1 })
    const base = ellipse(O3, 2, 2 / 3)
    const N2 = point(2 * rapport.num / rapport.den, -6 * (1 - rapport.num / rapport.den))
    const M4 = point(-2 * rapport.num / rapport.den, -6 * (1 - rapport.num / rapport.den))
    ...
    fl2.styleExtremites = '<->'
    fl3.styleExtremites = '<->'
    const leg1 = placeLatexSurSegment(`${texNombre(r, 1)}\\text{~cm}`, fl1G, fl1D, { distance: 0.3, letterSize: 'footnotesize' })
    const leg2 = placeLatexSurSegment(`${texNombre(h, 1)}\\text{~cm}`, fl2H, fl2B, { distance: 0.3, letterSize: 'footnotesize' })
    const leg3 = placeLatexSurSegment(`${texNombre(h * rapport.num / rapport.den, 1)}\\text{~cm}`, fl3H, fl3B, { distance: 0.3, letterSize: 'footnotesize' })
    const objets3 = [tri34, cone3, base, cone2, s3, s2, h3, angDr2, angDr3, fl1, fl2, fl3, leg1, leg2, leg3]
    const figure3 = mathalea2d(Object.assign({ pixelsParCm: 30, scale: 1 }, fixeBordures(objets3)), objets3)
    ...
    figure3, { largeur1: 65, widthmincol1: '500px', widthmincol2: '200px' })
...
: `Il a donc tort dans son estimation : si la hauteur de bonbons atteint ${niveau} de la hauteur du chapeau, alors le volume de bonbons est supérieur à $${pourcentage}\\,\\%$ du volume total de son chapeau.`}`
    ...
    const listeQuestionsB = createList({
      items: [`${context.isHtml ? '<br><br>' : '\\medskip\n'}${question4}`, question5],
      style: 'nombres'
    })
  ...
  versionAleatoire: () => void = () => {
    ...
    this.appliquerLesValeurs(r, h, tt, rapport, pourcentage, Array.isArray(quidam) ? quidam[0] : quidam)
  }
}
