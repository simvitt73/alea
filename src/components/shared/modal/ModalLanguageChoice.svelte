<script lang="ts">
  ...
  interface Props {
    showLanguageChoiceModal: boolean;
    handleLanguage: (lang: string) => void;
    locale: Language;
  }

  let { showLanguageChoiceModal = $bindable(), handleLanguage, locale }: Props = $props();
</script>
...
>
  {#snippet header()}
    <div  class="text-2xl w-full">Langue du Référentiel</div>
  {/snippet}
  {#snippet content()}
    <div >
