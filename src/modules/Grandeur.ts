...
class Grandeur {
  ...
  convertirEn(uniteConversion: string) {
    ...
    if (
      ...
      // Mêmes unités
      return new Grandeur(
        this.mesureDecimal.times(
          10 **
            ((this.puissancePrefixe - uniteConversionParsee.puissancePrefixe) *
              this.puissanceUnite),
        ),
        uniteConversion,
      )
      ...
      // On met tout en litres.
      return new Grandeur(
        this.mesureDecimal.times(
          10 **
            (this.puissancePrefixe -
              uniteConversionParsee.puissancePrefixe -
              3) *
            this.puissanceUnite,
        ),
        'L',
      ...
      return new Grandeur(
        this.mesureDecimal.times(
          10 ** (this.puissancePrefixe * this.puissanceUnite + 3),
        ),
        'm^3',
      )
    } else if (
      ...
      // On met tout en m².
      return new Grandeur(
        this.mesureDecimal.times(10 ** (this.puissancePrefixe + 2)),
        'm^2',
      )
    } else if (
      ...
      return new Grandeur(
        this.mesureDecimal.times(
          10 ** (this.puissancePrefixe * this.puissanceUnite - 2),
        ),
        'a',
      )
    } else
...
function parseUnite(unite: string) {
  ...
  const prefixe =
    unite === '°C'
      ...
      : ['t', 'q'].includes(unite)
        ? unite
        : avantPuissanceUnite.substring(0, avantPuissanceUnite.length - 1) // Pour prendre en compte la tonne aussi.
