...
function setList(exos) {
  if (!Array.isArray(exos))
    return console.error(Error('Il faut passer une liste d’exercices'))
  ...
  listeExos = exos.map((exo, i) => {
    try {
      ...
      if (
        !Array.isArray(exo.dimensionsDivMg32) ||
        exo.dimensionsDivMg32.length !== 2
      )
        throw Error('propriété dimensionsDivMg32 invalide')
      exo.dimensionsDivMg32.forEach((dim) => {
        if (typeof dim !== 'number' || dim < minSize || dim > maxSize)
          throw Error(
            `dimensionsDivMg32 doit contenir deux number entre ${minSize} et ${maxSize}`,
          )
      })
      if (typeof exo.MG32codeBase64 !== 'string' || !exo.MG32codeBase64)
        throw Error('propriété MG32codeBase64 invalide')
      return exo
    } catch (error) {
  ...
  if (mtgApp) {
    ...
    if (mtgApp.docs.length)
      console.error(
        Error('Après purge des docs mathgraph il en reste encore'),
        mtgApp.docs,
      )
  }
}
...
async function mg32Display(container, exo) {
  const {
    dimensionsDivMg32: [width, height],
    MG32codeBase64,
    mg32Editable,
    MG32codeBase64corr,
  } = exo
  ...
  if (indexExo === -1) {
    ...
    if (chunks) {
      ...
      while (
        document.getElementById(`MG32div${indexExo}`) &&
        indexExo < maxFigs
      )
        indexExo++
      if (indexExo === maxFigs)
        throw Error(`Trop de figures mathgraph dans le dom, ${maxFigs} max`)
    }
  }
  ...
  if (idDocCorr) {
    ...
    if (container) {
      ...
      console.error(
        Error(
          `Pas trouvé d’élément #MG32divcorr${indexExo} dans le dom, abandon de l'affichage de la correction`,
        ),
      )
    }
  }
  ...
  */
  if (typeof exo.mg32init === 'function')
    await exo.mg32init(mtgApp, idDoc, idDocCorr)
  return mtgApp
}
