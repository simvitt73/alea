...
export function listeQuestionsToContenuSansNumero (exercice: Exercice, retourCharriot = true) {
  const supprimerReference = document.getElementById('supprimer_reference')
  if (supprimerReference instanceof HTMLInputElement && supprimerReference.checked === true) {
 ...
 */
export function contraindreValeur (min: number, max: number, valeur: number, defaut: number) {
...
type gestionnaireFormulaireTexteParams = {
  saisie?: string | number, // Ce qui vient du formulaireTexte donc une série de nombres séparés par des tirets ou un seul nombre (normalement en string) ou rien
  min?: number,
  max?: number, // Obligatoirement > min
  defaut?: number, // Obligatoirement compris entre min et max inclus ou alors égal à melange
  listeOfCase?: string[] | number[], // La liste des valeurs à mettre dans la liste en sortie. Si aucune liste n'est fournie, ce sont les nombres qui seront dans la liste. La première valeur de listeOfCase correspond à la saisie numérique min et listeOfCase doit contenir max-min+1 valeurs
  shuffle?: boolean, // Si true, on brasse la liste en sortie sinon on garde l'ordre
  nbQuestions?: number, // Obligatoire : c'est la taille de la liste en sortie. Si 0, alors le nbQuestions correspond à la longueur de saisie.
  melange?: number, // La valeur utilisée pour l'option mélange
  enleveDoublons?: boolean, // Si true, la liste en sortie ne peut pas contenir deux fois la même valeur
  exclus?: number[] // Liste de valeurs à exclure entre min et max
}

export function gestionnaireFormulaireTexte (params: gestionnaireFormulaireTexteParams) {
  let { saisie, min, max, defaut, listeOfCase, shuffle, nbQuestions, melange, enleveDoublons, exclus } = params
  min = min ?? 1
  max = max ?? min
  if (max < min) max = min
  shuffle = shuffle ?? true
  enleveDoublons = enleveDoublons ?? false
  if (exclus) {
  ...
  if (typeof (saisie) === 'boolean' || saisie === '' || saisie === null || saisie === undefined) { // Si aucune liste n'est saisie
    ...
    if (typeof (saisie) === 'number' || Number.isInteger(saisie)) { // Si c'est un nombre, c'est que le nombre a été saisi dans la barre d'adresses
      listeIndex = [contraindreValeur(Math.min(min, melange ?? min), Math.max(max, melange ?? max), Number(saisie), defaut)]
    } else {
 ...
 * @author Rémi Angot
 */
export const ppcm = (a: number, b: number) => {
...
export function randint (min: number | Decimal, max: number | Decimal, listeAEviter: number[] | string = []) {
  ...
  if (typeof listeAEviter === 'string') {
    listeAEviter = listeAEviter.split('').map(Number)
  }
  ...
  if (listeAEviter.length > 0) {
    ...
    if (cpt === 50) {
      window.notify(`Randint n'a pas pu trouver de valeur en dehors de la liste à éviter, c'est donc cette valeur qui a été choisie : ${min + rand}`, { min, max, listeAEviter })
    }
  }
  return min + rand
}
 ...
 * @author Rémi Angot
 */
export function texParagraphe (liste: string[], spacing = 0, retourCharriot: boolean) {
  let result = ''
  if (spacing > 1) {
    result = `\\begin{spacing}{${spacing}}\n`
  }
...
export function printlatex (e: string) {
