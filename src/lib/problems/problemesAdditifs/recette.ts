...
export function recette1 (decimal = false): Probleme {
  ...
  let masse1 = randint(recette.ingredient1.masseMini, recette.ingredient1.masseMaxi)
  let masse2 = randint(recette.ingredient2.masseMini, recette.ingredient2.masseMaxi)
  if (decimal) {
...
Donc, la masse totale des ingrédients utilisés est de : $${texNombre(masse1, 1)}\\text{ ${unite}}+${texNombre(masse2, 1)}\\text{ ${unite}}=${miseEnEvidence(texNombre(masse1 + masse2, 1))}$ ${uniteComplete}.`
  ...
  probleme.schema.lignes[0].barres[0].content = `$${texNombre(masse1 + masse2, 2)}\\text{ ${unite}}$`
