...
export function papier (decimal = false): Probleme {
  ...
  const enonce = `Pour préparer ${choice(['une exposition', 'la rentrée', 'une sortie scolaire'])}, ${personnage.prenom} a utilisé ${nbFois} ramettes de papier, chaque ramette pesant $${texNombre(facteur, 1)}$ ${unite}. Quelle masse totale de papier a-t-${personnage.pronom} utilisé ?`
  const correction = `${personnage.prenom} a utilisé ${nbFois} ramettes de $${texNombre(facteur, 1)}$ ${unite}.
La masse totale de papier est : $${texNombre(nbFois, 0)}\\times ${texNombre(facteur, 1)}=${miseEnEvidence(texNombre(nbFois * facteur, 1))}$ ${uniteComplete}.`
