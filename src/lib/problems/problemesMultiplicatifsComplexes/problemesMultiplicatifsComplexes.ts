...
export default class ProblemeMultiplicatifsComplexes extends Probleme {
  constructor (name: string = '', data?: { nb1Fois: number, part1: number, part2: number }) {
    ...
    super(name, data)
    const longueurs = (nb2Fois: number): [number, number, number] => { // la longueur de la flèche si < puis la longueur de chaque part2 puis la longueur de la flèche si =
    ...
    this.schema = new SchemaEnBoite({
      topBraces: [{
        start: 1,
        end: data.nb1Fois * data.part1 === data.part2 * nb2Fois ? lengths[2] + 2 : lengths[0] + 2,
      ...
      lignes: [
        {
          barres: [
            ...
            {
              content: '',
              length: (data.nb1Fois * data.part1 === data.part2 * nb2Fois ? lengths[2] : lengths[0]) - 1,
              color: 'lightgray',
              type: 'flèche'
            }
          ]
      ...
      },
      {
        ...
        text: `$${texNombre(data.part2 * nb2Fois, 0)}$ cm`
      }]
    })

    this.enonce = `La documentaliste du collège doit couvrir $${data.nb1Fois}$ livres. Il faut $${data.part1}$ cm de papier kraft pour couvrir un livre.<br>
    ...
    this.correction = `Pour couvrir ${data.nb1Fois} livres, il en faut $${data.nb1Fois}\\times ${data.part1}$, soit $${data.nb1Fois * data.part1}$ cm de papier kraft.<br>
    Un rouleau de papier kraft mesure $${data.part2}$ m.<br>
