...
export class Rapporteur extends ObjetMathalea2D {
  ...
  constructor ({
    ...
    azimut = rotationAbstraite(point(this.x + 1, this.y), centre, depart)
    ...
    this.objets.push(segment(azimut2, rotationAbstraite(azimut2, centre, 180), color))
    ...
    for (let i = 0; i < nbDivisions; i++) {
      if (avecNombre !== '') {
        if (avecNombre === 'deuxSens') {
          if (i === 0) {
            numero = texteParPoint(String(arcPlein), rotationAbstraite(homothetie(azimut2, centre, 0.8), centre, 2), -depart, color, 0.65)
            numero.contour = true
            this.objets.push(numero)
          }
          if (i === nbDivisions - 1) {
            numero = texteParPoint(String(arcPlein - (1 + i) * 10), rotationAbstraite(homothetie(azimut2, centre, 0.8), centre, arcPlein / nbDivisions - 2), -depart, color, 0.65)
            numero.contour = true
            this.objets.push(numero)
          } else if ((arcPlein - (1 + i) * 10) % stepGraduation === 0) {
            numero = texteParPoint(String(arcPlein - (1 + i) * 10), rotationAbstraite(homothetie(azimut2, centre, 0.8), centre, arcPlein / nbDivisions), 90 - (1 + i) * 10 - depart, color, 0.78)
            numero.contour = true
            this.objets.push(numero)
          }
        }
        if (i === 0) {
          numero = texteParPoint('0', rotationAbstraite(homothetie(azimut2, centre, 0.9), centre, 2), -depart, color, 0.65)
          numero.contour = true
          this.objets.push(numero)
        }
        if (i === nbDivisions - 1) {
          numero = texteParPoint(String((1 + i) * 10), rotationAbstraite(homothetie(azimut2, centre, 0.9), centre, arcPlein / nbDivisions - 2), -depart, color, 0.65)
          numero.contour = true
          this.objets.push(numero)
        } else if ((i + 1) * 10 % stepGraduation === 0) {
          numero = texteParPoint(String((1 + i) * 10), rotationAbstraite(homothetie(azimut2, centre, 0.9), centre, arcPlein / nbDivisions), 90 - (1 + i) * 10 - depart, color, 0.65)
          numero.contour = true
          this.objets.push(numero)
        }
      }
      for (let s = 1, r; s < 10; s++) {
        if (s === 5 && precisionAuDegre < 10) {
          r = segment(homothetie(rotationAbstraite(azimut2, centre, s), centre, 0.92), homothetie(rotationAbstraite(azimut2, centre, s), centre, 0.99), color)
          r.opacite = 0.6
          r.tailleExtremites = 0.65
          this.objets.push(r)
        } else if (precisionAuDegre === 1) {
          r = segment(homothetie(rotationAbstraite(azimut2, centre, s), centre, 0.96), homothetie(rotationAbstraite(azimut2, centre, s), centre, 0.99), color)
          r.opacite = 0.6
          r.tailleExtremites = 0.65
          this.objets.push(r)
        }
    ...
    if (!semi) {
      rayon = segment(homothetie(rotationAbstraite(azimut, centre, -90), centre, -0.2), homothetie(rotationAbstraite(azimut, centre, -90), centre, 0.2), color)
      this.objets.push(rayon)
      rayon = segment(homothetie(azimut, centre, -0.2), homothetie(azimut, centre, 0.2), color)
    } else {
      rayon = segment(centre, homothetie(rotationAbstraite(azimut, centre, -90), centre, 0.2), color)
    }
    this.objets.push(rayon)
